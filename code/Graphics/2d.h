 ...
 *
 * $Log: /Freespace2/code/Graphics/2d.h $
 * 
...
#define gr_init_alphacolor2	GR_CALL(gr_screen.gf_init_alphacolor)
static void gr_init_alphacolor( color * dst, int r, int g, int b, int alpha, int type=AC_TYPE_HUD )
{
	gr_init_alphacolor2(dst, r, g, b, alpha, type);
}
...
#define gr_set_bitmap2		GR_CALL(gr_screen.gf_set_bitmap)
static void gr_set_bitmap( int bitmap_num, int alphablend=GR_ALPHABLEND_NONE, int bitbltmode=GR_BITBLT_MODE_NORMAL, float alpha=1.0f, int sx = -1, int sy = -1 )
{
	gr_set_bitmap2(bitmap_num, alphablend, bitbltmode, alpha, sx, sy);
}
...
#define gr_fog_set2			GR_CALL(gr_screen.gf_fog_set)
static void gr_fog_set(int fog_mode, int r, int g, int b, float fog_near = -1.0f, float fog_far = -1.0f)
{
	gr_fog_set2(fog_mode, r, g, b, fog_near, fog_far);
}
...
#define gr_tcache_set2		GR_CALL(gr_screen.gf_tcache_set)
static int gr_tcache_set(int bitmap_id, int bitmap_type, float *u_scale, float *v_scale, int fail_on_full = 0, int sx = -1, int sy = -1, int force = 0)
{ 
	return gr_tcache_set2(bitmap_id, bitmap_type, u_scale, v_scale, fail_on_full, sx, sy, force); 
}
